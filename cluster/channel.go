...
// Channel允许客户端发送特定状态类型的消息，该消息将以最佳方式被广播。
type Channel struct {
	key          string
	send         func([]byte)
	// peers 用来获取当前集群中的所有节点列表
...
// NewChannel创建一个新的Channel结构，用于处理向peer发送普通的和超大的消息。
func NewChannel(
	key string,
	send func([]byte),
	peers func() []*memberlist.Node,
...
// handleoversizemessages防止成员列表打开太多的并行TCP连接到它的对等端。
		...
		case b := <-c.msgc:
			...
			for _, n := range c.peers() {
				...
				go func(n *memberlist.Node) {
					...
					start := time.Now()
					// sendOversize 使用tcp发送信息，不使用gossip机制
